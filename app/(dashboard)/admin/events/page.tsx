        ...
        <div className="space-y-3">
          {events.map((ev) => {
            ...
            return (
              <div
                key={ev.id}
                className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden flex flex-col sm:flex-row"
              >
                {ev.cover_url && (
                  <img src={ev.cover_url} alt={ev.title} className="h-36 sm:h-auto w-full sm:w-24 md:w-32 object-cover shrink-0" />
                )}
                <div className="flex-1 p-4 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 min-w-0">
